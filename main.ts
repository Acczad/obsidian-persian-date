...
export default class InsertJalaliDatePlugin extends Plugin {
  onload() {
  ...
  insertJalaliDate() {
    ...
    if (!activeEditor) {
      new Notice('No active editor found. Please open a markdown file.');
      return;
    }
  ...
  gregorianToJalali(gregorianDate: Date): { year: number; month: number; day: number } {
    ...
    const yearOffsets: { [key: number]: number } = {
  ...
  dayOfYearToJalali(dayOfYear: number, jalaliYear: number): { month: number; day: number } {
  ...
  isLeapYear(jalaliYear: number): boolean {
